...
for noduleType in noduleTypes:
    ...
    for idx1, f in enumerate(range(len(npzs[0]))):
        ...
        views = get_ortogonal_patches(npz['data'], r=0)
        for view in views:
            trainX.append(view)
            trainY.append(i)
...
def split_training_validation_datasets(x, y, val_percentage=0.3, val_balanced=True):
    ...
    if val_balanced == False:
        ...
        all_idx = np.arange(olen)
        val_idx=[]
        nitems = int((collections.Counter(y).most_common()[-1][1]) * 0.3)/45
        for c in np.unique(y):
            idx = np.random.choice(np.where(orgy==c)[0], nitems, replace=False)
            val_idx.append(np.hstack([np.arange(xx*45,45*xx+45) for xx in idx]))
        all_idx = np.arange(len(y))        
...
def build_neural_network(data_size, n_classes):
    ...
    network = lasagne.layers.Conv2DLayer( network, num_filters=64, filter_size=(3, 3),  nonlinearity=lasagne.nonlinearities.LeakyRectify(0.33), W=lasagne.init.GlorotUniform()) 
#    network = lasagne.layers.batch_norm(network)
    network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
    network = lasagne.layers.Conv2DLayer( network, num_filters=32, filter_size=(3, 3),  nonlinearity=lasagne.nonlinearities.LeakyRectify(0.33), W=lasagne.init.GlorotUniform()) 
    network = lasagne.layers.batch_norm(network)
    network = lasagne.layers.MaxPool2DLayer(network, pool_size=(2, 2))
    network = lasagne.layers.DenseLayer( network, num_units=1024, W=lasagne.init.GlorotUniform(),      nonlinearity=lasagne.nonlinearities.rectify)
...
loss = loss.mean()
layers = {network.input_layer.input_layer: 0.005}#, network.input_layer.input_layer.input_layer.input_layer: 0.01}
...
n_epochs   = 100
batch_size = 32 # adapt this value based on the memory of your GPU
...
test_prediction = voting(test_prediction,45)
