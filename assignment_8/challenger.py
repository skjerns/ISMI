...
def submit_results(user, csv_results, description=None):
    ...
    upload_challenge_data(client, user, 'amida', csv_results, metadata=description)
    print(
...
def create_results_csv(results_file, results_folder):
    ...
    filedata[0] = 'file_id,label\n'

    # Naive checking that we have results for everything
    if len(filedata) != 13202:
        raise(ValueError('You do not have the correct number of entries in your csv file. Do you have results for all of the test data?'))
    
    for line in filedata[1:]:
        # Check that the filename has the correct format
        file_id = line.split(',')[0]
        if not re.match('^([0-9]{2}_){2}Normalized_[0-9]{2,}_[0-9]{2,}$', file_id):
            raise(ValueError('The format of the filename is not correct, it should be in the format of\n   00_00_Normalized_00_00\nyours is: \n   %s'%file_id))

        # Check that the label can be converted to an int
        try:
            int(line.split(',')[1])
        except ValueError:
            raise(ValueError('The format of your csv file is incorrect. It should be comma separated, the 1st column is filename, the 2nd the label. We could not convert your label to an int.'))
